...
export const projectDetails: Record<string, DetailExtras> = {
  "mobile-app-redesign": {
    ...
    tasks: [
      { id: "m_k1", title: "Finalize the onboarding flow", status: "in_review", priority: "high", assigneeId: "u_maya", due: "Today", group: "This week", description: "Lock the five-step first-run experience: copy, edge cases, and the skip path. This is the last blocker before Noah can start the final integration pass, so it needs sign-off today." },
      { id: "m_k2", title: "Integrate new tab bar animations", status: "in_progress", priority: "high", assigneeId: "u_noah", due: "Fri", group: "This week", description: "Wire the spring transitions from the navigation prototype into the real tab bar, including the shared-element handoff between tabs." },
      { id: "m_k3", title: "Audit empty states across screens", status: "todo", priority: "medium", assigneeId: "u_ines", due: "Aug 8", group: "Next week", description: "Catalogue every list and detail screen that can render with zero items and make sure each has a designed empty state rather than a blank panel." },
      { id: "m_k4", title: "Wire up haptics for key actions", status: "todo", priority: "low", assigneeId: "u_noah", group: "Next week" },
      { id: "m_k5", title: "Set up dark-mode snapshot tests", status: "done", priority: "medium", assigneeId: "u_ada", group: "Done", description: "Add snapshot coverage for the core screens in dark mode so token regressions get caught in CI." },
    ],
    documents: [
      { id: "m_d1", title: "Onboarding flow: final spec", kind: "spec", excerpt: "The five-step first-run experience, with copy and edge cases.", ownerId: "u_maya", updatedAt: "2026-08-01T13:40:00Z", body: "## Goal\nGet a new user to their first meaningful action in **under 60 seconds**.\n\n## The five steps\n1. Welcome — one sentence on what Orbit does, and a single primary button.\n2. Workspace — name it, or accept the suggested name.\n3. Invite — optional; skippable without friction.\n4. First project — pick a template or start blank.\n5. Done — drop straight into the workspace, not a dashboard tour.\n\n## Edge cases\n- Returning user on a new device: skip to step 4.\n- No network mid-flow: keep entered data and retry silently.\n- Skip path must never dead-end; every step has a visible way out." },
      { id: "m_d2", title: "Navigation model", kind: "design", excerpt: "Tab structure, gestures, and the transition system.", ownerId: "u_ines", updatedAt: "2026-07-29T16:10:00Z", body: "## Structure\nFive primary tabs, fixed order, no overflow menu. The active tab owns the accent.\n\n## Gestures\n- Horizontal swipe moves between sibling tabs.\n- Long-press a tab for quick actions (new item in that section).\n\n## Transitions\nShared-element handoff for the tab indicator; content cross-fades with a 4px rise. Motion stays in the `150–220ms` band so it reads as feedback." },
      { id: "m_d3", title: "Release checklist v3.0", kind: "doc", excerpt: "Everything that must be green before TestFlight.", ownerId: "u_ada", updatedAt: "2026-07-31T11:00:00Z", body: "## Before TestFlight\n- [ ] All P0/P1 bugs closed or waived by the lead.\n- [x] Dark-mode snapshots green in CI.\n- [ ] Onboarding copy signed off.\n- [ ] Crash-free sessions > 99.5% on the internal build.\n- [ ] Release notes drafted and reviewed.\n\n## Sign-off\nEng lead **and** product lead both approve in the release thread." },
    ],
    feed: [
